...
var (
	...
	calcPos       string              // calculated positions
	places        = make(map[int]int) //places[1] returns dancer number in 1st or left dancer, init should be 1:1 2:2 3:3
	...
	for {
		...
		case pos := <-posChan:
			...
			if !allDancersIdentified {
				dancerNoToPlace[posPack1.dancerNo] = 1
				dancerNoToPlace[posPack2.dancerNo] = 2
				dancerNoToPlace[posPack3.dancerNo] = 3
			...
			dancerNoToPlace[posPack1.dancerNo] = dancerNoToPlace[posPack1.dancerNo] + posPack1.change
			dancerNoToPlace[posPack2.dancerNo] = dancerNoToPlace[posPack2.dancerNo] + posPack2.change
			dancerNoToPlace[posPack3.dancerNo] = dancerNoToPlace[posPack3.dancerNo] + posPack3.change

			calcPos = fmt.Sprint(dancerNoToPlace[posPack1.dancerNo], dancerNoToPlace[posPack2.dancerNo], dancerNoToPlace[posPack3.dancerNo])
...
func init() {
	...
	flag.StringVar(&mode, "mode", "single", "Enter mode: single or multi, defaults to single.")
